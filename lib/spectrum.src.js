...
function SpectrumError (message, type) {
...
var topLevelRule = rule{
    ...
    inheritDirectiveRule = rule{
        (\s+)             // pre whitespace
        (?:
            none
        |   \"(\S+?)\"       // what we inherit from
        )
        (\s*)/>          // posible whitespace and end of unary tag
    }x,
    initBlockRule = rule{
        >([\S\s]*?)</~init>   // content of the init block
    }x,
    beginBlockRule = rule{
        >([\S\s]*?)</~begin>  // content of begin block
    }x;
    ...
    return fs.readFile(this.sourceRoot + path, 'utf8').then(
        ...
        function (err) {
            ...
            throw new SpectrumError(
                'could not load template: ' + err.message,
                errors.couldNotReadTemplate
            );
        }
    );
};
