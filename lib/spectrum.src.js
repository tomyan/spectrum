...
Root.prototype.setInheritDirective = function (node) {
    if (this._inheritDirective) {
        throw new Error('only one <~inherit ... > directive per template allowed');
    }
    this._inheritDirective = node;
};
...
var topLevelRule = rule{
    ...
    initBlockRule = rule{
        >(.*?)<\/~init>   // content of the init block
    }x,
    beginBlockRule = rule{
        >(.*?)<\/~begin>  // content of begin block
    }x;
